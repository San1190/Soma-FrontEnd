...
const PillInput = ({ label, value, onChangeText, smallWidth = false, editable = true, ...props }) => {
  const styles = StyleSheet.create({
    pillInputContainer: { 
        // Si es smallWidth, ocupa la mitad del ancho del contenedor padre (dataRow)
        width: smallWidth ? '48%' : '100%', 
        marginBottom: 15,
        marginRight: smallWidth ? '4%' : 0, // Espacio entre dos inputs
    },
    label: { 
        fontSize: 14, 
        color: '#666', 
    ...
    input: {
        backgroundColor: '#E6E0F5', // Lila/Lavanda para el input de datos
        borderRadius: 12, 
        ...
        fontWeight: '600', 
        color: '#333333',
        // Estilo estático si no es editable, como en la imagen
...
const PersonalizationScreen = ({ navigation }) => {
  ...
  const styles = StyleSheet.create({
    ...
    scrollContainer: {
      ...
      // Damos un padding generoso para el teclado y el gato
    ...
    catImageContainer: {
      ...
      // ELIMINADO: backgroundColor: DARK_SILHOUETTE_COLOR, 
      // Ahora solo la imagen PNG/SVG (gatoCola.png) define el color.
      zIndex: 1, 
    },
    ...
    nextButton: {
      ...
      position: 'absolute', // Sigue siendo absoluto, posicionado sobre el gato
      bottom: CAT_IMAGE_HEIGHT / 2 + 50, // Lo movemos hacia arriba, lejos del fondo
  ...
  return (
    <View style={styles.outerContainer}>
      ...
      >
        ...
        {/* Fondo Fijo: Imagen del Gato en la parte inferior (CORREGIDO) */}
